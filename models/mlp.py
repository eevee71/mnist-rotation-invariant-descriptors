...
def train_mlp(
    ...
    """Trains an MLP on rotation invariants.

    Model selection is done on a validation split carved out of the TRAINING
    set. The rotated test set is evaluated exactly once, at the end, using
    the selected weights. Selecting on the test set (as an earlier version
    did) reports the maximum over epochs of test accuracy, which is an
    optimistically biased estimate, not a held-out one.
    """
    ...
    Xtr, Xte, ytr, yte, _ = prepare_pipeline(
        splits, degree=degree, k=k, rot_seed=rot_seed
    )

    # validation split carved from training data only
    Xtr, Xval, ytr, yval = train_test_split(
        Xtr, ytr, test_size=val_frac, random_state=seed, stratify=ytr
    )
    print(f"train: {Xtr.shape[0]} | val: {Xval.shape[0]} | test: {Xte.shape[0]}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # model selection on the validation split -- the test set is not touched here
        model.eval()
        with torch.no_grad():
            val_acc = (model(Xval_t).argmax(dim=1) == yval_t).float().mean().item()
    ...
    print(f"--- Training Finished. Best Val Accuracy: {best_val_acc * 100:.2f}% (Epoch {best_epoch}) ---")

    # single, final evaluation on the rotated test set
    ...
    print(f"--- Held-out rotated test accuracy: {test_acc * 100:.2f}% ---")

    yte_np = yte_t.cpu().numpy()
    return model, yte_np, test_preds.cpu().numpy()